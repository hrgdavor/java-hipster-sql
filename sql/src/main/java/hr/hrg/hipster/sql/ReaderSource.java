...
public class ReaderSource {
	...
	protected Map<Class<? extends Object>, IReadMeta<?>> registered = new ConcurrentHashMap<>(); 
	...
	public ReaderSource(HipsterSql hip){
		this.hip = hip;
	}

	public <T> void registerFor(IReadMeta<T> handler, Class<T> clazz){
		registered.put(clazz, handler);
	}

	public <T> void registerFor(IReadMeta<T> handler){
	 ...
	 */
	public <T> IReadMeta<T> getForRequired(Class<T> clazz) {
		IReadMeta<T> for1 = getFor(clazz);
	...
	public <T> IReadMeta<T> getFor(Class<T> clazz) {
		return (IReadMeta<T>) registered.get(clazz);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> IReadMeta<T> getOrCreate(Class<T> clazz) {
		IReadMeta<T> ret = (IReadMeta<T>) registered.get(clazz);
		if(ret == null){
			...
			registered.put(clazz, (IReadMeta<?>) ret);
		}
		return ret;
	}

	@SuppressWarnings("unchecked")
	<T> IReadMeta<T> newResultGetter(Class<T> clazz) {
		...
		try {
			...
			return (IReadMeta<T>) meta.getConstructor(new Class<?>[]{HipsterSql.class, int.class}).newInstance(hip, ordinalIndex++);
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if(idx != -1 && idx <methodName.length() && Character.isUpperCase(methodName.charAt(idx))){
				...
				ICustomType<?> getter = hip.getTypeSource().getForRequired(returnType, typeParams);
				getters.add(getter);
			}
		}

		return new ReaderMeta<>(clazz, "",columns, getters);
	}
	...
	private BaseColumnMeta<?> makeColumnMeta(Class<?> clazz, Method method, String methodName, String name, String tableName, int ordinal, Class<?> returnType, Class<?> ...typeParams) {
		...
		return new BaseColumnMeta(ordinalIndex++, name, columnName, methodName, meta, returnType, returnTypePrimitive, columnSql, null, typeParams); 
	}

...
public class ReaderSource {
	...
	protected Map<Class<? extends Object>, IReadMeta<?, ? extends IColumnMeta>> registered = new ConcurrentHashMap<>(); 
	...
	public <T> void registerFor(IReadMeta<T, ? extends IColumnMeta> handler, Class<T> clazz){
		registered.put(clazz, handler);
	}

	public <T> void registerFor(IReadMeta<T, ? extends IColumnMeta> handler){
	 ...
	 */
	public <T> IReadMeta<T, ? extends IColumnMeta> getForRequired(Class<T> clazz) {
		IReadMeta<T, ? extends IColumnMeta> for1 = getFor(clazz);
	...
	public <T> IReadMeta<T, ? extends IColumnMeta> getFor(Class<T> clazz) {
		return (IReadMeta<T, ? extends IColumnMeta>) registered.get(clazz);
	}
	...
	public <T, E extends IColumnMeta> IReadMeta<T, E> getOrCreate(Class<T> clazz) {
		...
		if(ret == null){
			...
			registered.put(clazz, (IReadMeta<?, IColumnMeta>) ret);
		}
		return ret;
	}
	...
	<T, E extends IColumnMeta> IReadMeta<T, E> newResultGetter(Class<T> clazz) {
		...
		try {
			...
			return (IReadMeta<T, E>) meta.getConstructor(new Class<?>[]{ResultGetterSource.class, int.class}).newInstance(resultGetterSource, ordinalIndex++);
		...
		List<IColumnMeta> columns = new ArrayList<>();
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if(idx != -1 && idx <methodName.length() && Character.isUpperCase(methodName.charAt(idx))){
				...
				IColumnMeta columnMeta = makeColumnMeta(clazz, methods[i], methodName, name, tableName, i, returnType, typeParams);

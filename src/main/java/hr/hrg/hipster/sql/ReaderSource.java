...
public class ReaderSource {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> IReadMeta<T, ResultColumnMeta> getOrCreate(Class<T> clazz) {
		IReadMeta<T, ResultColumnMeta> ret = (IReadMeta<T, ResultColumnMeta>) registered.get(clazz);
		if(ret == null){
	...
	<T> IReadMeta<T, ResultColumnMeta> newResultGetter(Class<T> clazz) {

		Method[] methods = clazz.getMethods();
		List<ResultColumnMeta> columns = new ArrayList<ResultColumnMeta>();
		String tableName = "";
		for (int i = 0; i < methods.length; i++) {
			...
			if(idx != -1 && idx <methodName.length() && Character.isUpperCase(methodName.charAt(idx))){
				...
				columns.add(makeColumnMeta(clazz, methods[i], methodName, name, tableName, i));
			}
		}
		
		ReaderMeta<T> reader = new ReaderMeta<>(clazz, tableName, columns);
		return reader;
	}
